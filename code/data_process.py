...
def process_dataset(dataset, window=110, speaker_task='True', demons='False', predictions='True'):
    ...
    label_set = {
        ...
        'dailydialog':['happiness', 'no emotion', 'anger', 'sadness', 'fear', 'surprise', 'disgust']
    }
    label_text_set = {
        'iemocap':'happy, sad, neutral, angry, excited, frustrated',
        'meld'   :'neutral, surprise, fear, sad, joyful, disgust, angry',
    ...
    speaker_label_text_set = {
        ...
        'dailydialog':'Speaker_0, Speaker_1',
    }
    ...
    speaker_label_dict = {}
    speaker_label_int_dict = {}
    content_target_dict = {}
    speaker_target_dict = {}
    content_task_dict = {}
    speaker_task_dict = {}
    sentence_dict = {}
    personality_label_dict = {}
    data = pickle.load(open(f'../original_data/{dataset}/{dataset}_remake.pkl','rb'))
    ...
    if dataset == 'iemocap':
        ...
        profiles = json.load(open(f'../original_data/{dataset}/profiles.json'))
        for i in profiles['profiles']:
            personality_label_dict[i['mbti_profile']]=i['personality_type'].split()[0]
        all_conv_id = data[4] + data[5] + data[6]
        sentence_dict = data[3]
        for conv_id in all_conv_id:
            temp_speaker_list = []
            temp_speaker_int_list = []
            temp_speaker_conv_dict={}
            i=0
            for speaker_label in data[0][conv_id]:
                if speaker_label not in temp_speaker_conv_dict:
                    temp_speaker_conv_dict[speaker_label]=i
                    i=i+1
                #temp_speaker_list.append(speaker_label.index(1))
                temp_speaker_list.append(speaker_label)
                temp_speaker_int_list.append(temp_speaker_conv_dict[speaker_label])
            speaker_label_dict[conv_id] = temp_speaker_list
            speaker_label_int_dict[conv_id] = temp_speaker_int_list
    ...
    for conv_id in all_conv_id:
        for conv_turn in range(len(sentence_dict[conv_id])):
            ...
            for speaker_label, speaker_int_label, sub_sent in zip(speaker_label_dict[conv_id][index_w:conv_turn+1], speaker_label_int_dict[conv_id][index_w:conv_turn+1], sentence_dict[conv_id][index_w:conv_turn+1]):
#                temp_content_str += (f'\t Speaker_{speaker_int_label}:"{sub_sent}"')
                temp_content_str += (f'\t {speaker_label}:"{sub_sent}"')
#                try:
#                    temp_content_str += (f'\t Speaker_{personality_label_dict[speaker_label]}:"{sub_sent}"')
#                except:
#                    temp_content_str += (f'\t Speaker_{speaker_label}:"{sub_sent}"')
            ...
            temp_content_str += ' ### '
#            try:
#                temp_content_str += f'<{target_utterance.split(":")[0]}> has personality type <{personality_label_dict[speaker_label_dict[conv_id][conv_turn]]}>. Please select the emotional label of <{target_utterance}> from <{label_text_set[dataset]}>. This is very important to my career.'
#            except:
#                temp_content_str += f'<{target_utterance.split(":")[0]}> has personality type <Unknown>. Please select the emotional label of <{target_utterance}> from <{label_text_set[dataset]}>. This is very important to my career.'
            temp_content_str += f'Please select the emotional label of <{target_utterance}> from <{label_text_set[dataset]}>.'# This is very important to my career.'
            # demon
            ...
            content_task_dict[f'{conv_id}_{conv_turn}'] = temp_content_str#.upper()

    for conv_id in all_conv_id:
        for conv_turn in range(len(data[2][conv_id])):
            temp_speaker_task_str = f'Now you are EXPERT of SENTIMENT and EMOTIONAL ANALYSIS. Please select the SPEAKER LABEL of the utterance <Speaker: {sentence_dict[conv_id][conv_turn]}> from <{speaker_label_text_set[dataset]}>. This is very important to my career.'
