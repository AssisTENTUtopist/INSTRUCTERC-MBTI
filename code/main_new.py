...
def report_score(dataset, golds, preds, mode='test'):
    if dataset == 'iemocap':
        ...
        target_names = ['happiness', 'no emotion', 'anger', 'sadness', 'fear', 'surprise', 'disgust']
        digits = 7
    ...
    if dataset == 'dailydialog':
        res['Micro_F1_SA'] = f1_score(golds, preds, labels=[0,2,3,4,5,6], average='micro')
    res['mode'] = mode
...
parser.add_argument(
    ...
    type=str,
    required=True,
    choices=['iemocap','meld','EmoryNLP', 'dailydialog'],
...
if "chatglm" in args.model_name_or_path.lower():
    ...
    if "bloom" in args.model_name_or_path.lower() or "falcon" in args.model_name_or_path.lower():
        ## for bloom, falcon
        config = AutoConfig.from_pretrained(args.model_name_or_path, trust_remote_code=True)
        tokenizer = AutoTokenizer.from_pretrained(args.model_name_or_path, trust_remote_code=True)
        ...
        if "llama" in args.model_name_or_path.lower():
            print('***** Preparing ', args.model_name_or_path, ' as LLaMA model *****')
            ...
            config = AutoConfig.from_pretrained(args.model_name_or_path, trust_remote_code=True)
            model = AutoModelForCausalLM.from_pretrained(args.model_name_or_path, **params)
            tokenizer = AutoTokenizer.from_pretrained(args.model_name_or_path, trust_remote_code=True, use_fast=False)
            print('***** Prepared ', args.model_name_or_path, ' as LLaMA model *****')
        else:
    ...
    if args.do_train:
        ...
        for epoch in range(args.num_train_epochs):
            ...
            if args.statistic_mode == "True":
                ...
                if args.dataset == 'dailydialog':
                    if best_micro_f1_score < score['Micro_F1_SA']:
                        best_micro_f1_score = score['Micro_F1_SA']
                ...
                elif best_f1_score < score['F1_SA']:
